struct LocationServicesTermsModel{
    static let text = """
**제1조 (목적)**

본 약관은 회원(본 약관에 동의한 자를 말합니다. 이하 “회원”이라고 합니다)이 사부작(이하 “회사”라고 합니다)이 제공하는 위치 기반 서비스(이하 “서비스”라고 합니다)를 이용함에 있어 회사와 회원의 권리・의무 및 책임사항을 규정함을 목적으로 합니다.

**제2조 (약관의 효력 및 변경)**

1. 본 약관은 서비스를 신청한 고객 또는 개인 위치 정보 주체가 본 약관에 동의하고 회사가 정한 소정의 절차에 따라 서비스의 이용자로 등록함으로써 효력이 발생합니다.
...
2. 회사는 아래와 같은 경우 회원의 서비스 가입신청에 대한 승낙을 유보할 수 있습니다.
    - 실명이 아니거나 다른 사람의 개인정보를 사용하는 등 허위로 신청하는 경우
...
2. 회사는 단말기의 설정 상태에 따라, 회원이 앱을 구동 내지 사용하는 특정 시점 혹은 회원이 앱 내 특정 서비스를 활성화하거나 동네인증 등의 액션을 하는 시점에, 위치정보사업자로부터 위치정보를 위경도 좌표의 형식으로 전달받게 됩니다. 회사는 해당 위치정보를 사전에 정의된 특정 구역 단위 혹은 특정 행정동 단위에 속하는지 여부에 대한 값으로 변환할 수 있습니다.
...
서비스명 : 동네 인증
서비스 목적 및 내용 : ◦ 회원의 위치가 특정 구역 내지 행정동 단위에 속하는지 여부를 확인하여 ‘내 동네’ 인증 ◦ 예시: 인증된 ‘내 동네’를 바탕으로 모임추천, 플레이스 추천 제공등
보유기간 : 관련 법령 및 제5항이 정하는 바에 의함

서비스명 : 서비스 이용 편의 제공
서비스 목적 및 내용 : ◦ 홈화면 상단에 회원의 위치를 배치하여, 위치설정 변경의 편의성 증진 ◦ 예시: 플레이스 거리 및 정보 확인, 기타 지도 연동 서비스 이용 시, 회원의 위치정보에 기반한 서비스 편의 제공 등

서비스명 : 서비스 내 콘텐츠 제공 내지 추천
서비스 목적 및 내용 : ◦ 회원의 위치정보 혹은 이것이 특정 구역 내지 행정동 단위에 속하는지 여부를 기반으로, 앱 내 다양한 콘텐츠 제공 내지 추천에 활용 ◦ 예시: 회원 위치 기반 특정 반경 내 모임 추천, 회원 위치기반 취미 플레이스 추천, 기타 검색결과 활용 등

서비스명 : 서비스 제공 내용 고도화
서비스 목적 및 내용 : ◦ 회원의 위치정보에 대한 통계적 분석을 통해, 회원의 위치정보기반 콘텐츠 추천, 콘텐츠 표시 등 각종 응용 서비스 제공 및 고도화
...
2. 전항의 경우 불특정 다수인을 상대로 통지를 함에 있어서는 서비스 홈페이지 등 기타 회사의 공지사항 페이지를 통하여 회원들에게 통지할 수 있습니다. 단, 회원 본인의 서비스내 활동과 관련하여 중대한 영향을 미치는 사항은 상당한 기간 동안 서비스 홈페이지에 게시하거나 회원에게 전자적 형태(전자우편, SMS 등)로 개별통지 합니다.
...
7. 회사는 회원의 동의가 있거나 다음 각 호의 어느 하나에 해당하는 경우를 제외하고는 개인위치정보 또는 위치정보 이용∙제공사실 확인자료를 이용약관에 명시 또는 고지한 범위를 넘어 이용하거나 제3자에게 제공할 수 없습니다.
    1. 위치기반서비스의 제공에 따른  위치정보 이용∙제공사실 확인자료가 필요한 경우
...
1. 회사는 다음 각 호의 경우로 서비스를 제공할 수 없는 경우 이로 인하여 회원에게 발생한 손해에 대해서는 책임을 부담하지 않습니다.
    ...
    3. 회원의 귀책사유로 서비스 이용에 장애가 있는 경우4) 제1호 내지 제3호를 제외한 기타 회사의 고의ㆍ과실이 없는 사유로 인한 경우
...
제1조 (시행일) 본 약관은 2023년 12월 13일부터 시행합니다.
"""
}
